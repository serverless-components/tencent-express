...
const getFunctionState = async (clients, lambdaName) => {
	const scf = await tencentUtils.getTencentSCFByName(clients.scf, lambdaName)
	if (scf.Status == 'Updating' || scf.Status == 'Creating') {
		return true
	}
	return false
}
...
const createOrUpdateLambda = async (instance, inputs, clients) => {
	...
	let times = 200
	while (await getFunctionState(clients, instance.state.lambdaName)) {
		await sleep(51)
		times = times - 1
		status = (await getFunctionState(clients, instance.state.lambdaName)).Status
	}
